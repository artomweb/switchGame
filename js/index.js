...
function createGamePieces(pieces) {
    ...
    for (let i = 0; i < pieces.length; i++) {
        for (let j = 0; j < pieces[i].length; j++) {
            ...
            let thisPiece;
            if (i === 4) {
                thisPiece = new Piece(pieces[i][j].x, pieces[i][j].y, pieces[i][j].topColour, pieces[i][j].bottomColour, false, pieces[i][j].justFlipped);
            } else {
                let x = i * boxWidth + boxWidth / 2;
                let y = j * boxHeight + boxHeight / 2;
                thisPiece = new Piece(x, y, pieces[i][j].topColour, pieces[i][j].bottomColour, false, pieces[i][j].justFlipped);
            }
...
function draw() {
    ...
    for (let i = 0; i < boardPieces.length; i++) {
        for (let j = 0; j < boardPieces[i].length; j++) {
            ...
            boardPieces[i][j].drawPiece(flashItt);
        }
    }
